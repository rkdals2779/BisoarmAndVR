"""HMD·좌우 컨트롤러의 6-DoF pose를 실시간 시각화하는 진단 스크립트.

matplotlib 3D 축 화살표와 Yaw 추적 차트, 콘솔 멀티라인 텔레메트리로
SteamVR 트래킹 상태를 한눈에 확인한다.
"""
...
BUFFER_SIZE: Final[int] = 100  # Yaw 차트 표시 프레임 수
LOOP_INTERVAL_S: Final[float] = 0.03


def extract_euler_from_matrix(
	pose_matrix: 'openvr.HmdMatrix34_t',
) -> tuple[float, float, float]:
	"""OpenVR 3x4 변환 행렬에서 atan2 방식으로 Roll/Pitch/Yaw(도) 추출."""
	pitch = math.degrees(math.atan2(
		-pose_matrix[1][2],
		math.sqrt(pose_matrix[0][2] ** 2 + pose_matrix[2][2] ** 2),
	))
...
def update_quiver_axes(
	...
	"""변환 행렬 기반으로 3D 공간에 R(X), G(Y), B(Z) 화살표 축 플로팅."""
	quiver_x = ax.quiver(
		x, z, y, pose_matrix[0][0], pose_matrix[2][0], pose_matrix[1][0],
		color='red', length=scale, normalize=True, lw=1.5,
	)
	quiver_y = ax.quiver(
		x, z, y, pose_matrix[0][1], pose_matrix[2][1], pose_matrix[1][1],
		color='green', length=scale, normalize=True, lw=1.5,
	)
	quiver_z = ax.quiver(
		x, z, y, pose_matrix[0][2], pose_matrix[2][2], pose_matrix[1][2],
		color='blue', length=scale, normalize=True, lw=1.5,
	)
	quivers_list.extend([quiver_x, quiver_y, quiver_z])


def main() -> None:
	"""SteamVR 연결 후 3D/차트/콘솔 시각화 루프를 실행한다."""
	# 1. SteamVR 백엔드 초기화
	try:
	...
	fig.canvas.manager.set_window_title(
		'VR Full System 6-DoF Multi-Axis Analyzer'
	)
	...
	scatter_left = ax3d.scatter(
		[], [], [], c='darkred', s=40, label='Left Controller'
	)
	scatter_right = ax3d.scatter(
		[], [], [], c='darkblue', s=40, label='Right Controller'
	)
	...
	try:
		while plt.fignum_exists(fig.number):
			...
			if hmd_pose.bPoseIsValid:
				...
				hmd_x = pose_matrix[0][3]
				hmd_y = pose_matrix[1][3]
				hmd_z = pose_matrix[2][3]
				hmd_roll, hmd_pitch, hmd_yaw = (
					extract_euler_from_matrix(pose_matrix)
				)
				scatter_hmd._offsets3d = ([hmd_x], [hmd_z], [hmd_y])
				update_quiver_axes(
					ax3d, pose_matrix, hmd_x, hmd_y, hmd_z,
					active_quivers, scale=0.3,
				)
				hmd_status = 'ON '
			...
			if left_index is not None and poses[left_index].bPoseIsValid:
				...
				left_x = pose_matrix[0][3]
				left_y = pose_matrix[1][3]
				left_z = pose_matrix[2][3]
				left_roll, left_pitch, left_yaw = (
					extract_euler_from_matrix(pose_matrix)
				)
				scatter_left._offsets3d = ([left_x], [left_z], [left_y])
				update_quiver_axes(
					ax3d, pose_matrix, left_x, left_y, left_z,
					active_quivers, scale=0.2,
				)
				left_status = 'ON '
			else:
			...
			if right_index is not None and poses[right_index].bPoseIsValid:
				...
				right_x = pose_matrix[0][3]
				right_y = pose_matrix[1][3]
				right_z = pose_matrix[2][3]
				right_roll, right_pitch, right_yaw = (
					extract_euler_from_matrix(pose_matrix)
				)
				scatter_right._offsets3d = (
					[right_x], [right_z], [right_y]
				)
				update_quiver_axes(
					ax3d, pose_matrix, right_x, right_y, right_z,
					active_quivers, scale=0.2,
				)
				right_status = 'ON '
			else:
			...
			# 각 줄 끝의 \033[K 로 현재 줄의 이전 잔상을 깨끗하게 지움
			print(
				f'\n[{count:04d}] HMD    ({hmd_status}) -> '
				f'XYZ:[{hmd_x: .2f},{hmd_y: .2f},{hmd_z: .2f}] '
				f'RPY:[{hmd_roll: .1f},{hmd_pitch: .1f},'
				f'{hmd_yaw: .1f}]\033[K'
			)
			print(
				f'       L_Ctrl ({left_status}) -> '
				f'XYZ:[{left_x: .2f},{left_y: .2f},{left_z: .2f}] '
				f'RPY:[{left_roll: .1f},{left_pitch: .1f},'
				f'{left_yaw: .1f}]\033[K'
			)
			print(
				f'       R_Ctrl ({right_status}) -> '
				f'XYZ:[{right_x: .2f},{right_y: .2f},{right_z: .2f}] '
				f'RPY:[{right_roll: .1f},{right_pitch: .1f},'
				f'{right_yaw: .1f}]\033[K',
				end='', flush=True,
			)

			current_len = len(history_hmd_yaw)
			line_hmd_yaw.set_data(
				np.arange(current_len), list(history_hmd_yaw)
			)
			line_left_yaw.set_data(
				np.arange(current_len), list(history_left_yaw)
			)
			line_right_yaw.set_data(
				np.arange(current_len), list(history_right_yaw)
			)
