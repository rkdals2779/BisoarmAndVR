...
[수정] 이 팔은 이제 오른손이 아니라 왼손 VR 컨트롤러로 조작합니다.
바뀐 부분은 get_right_controller_idx -> get_left_controller_idx 함수
하나뿐이고(TrackedControllerRole_RightHand -> LeftHand), 나머지 로직/
로봇 포트/그리퍼 등은 전부 그대로입니다.
...
# 1. 설정값
# ============================================================
FOLLOWER_ARM_ID = "skm_left_follower"
...
# 4. VR 헬퍼 함수
# ============================================================
def get_left_controller_idx(vr_system):
    for i in range(openvr.k_unMaxTrackedDeviceCount):
        if vr_system.getTrackedDeviceClass(i) == openvr.TrackedDeviceClass_Controller:
            if vr_system.getControllerRoleForTrackedDeviceIndex(i) == openvr.TrackedControllerRole_LeftHand:
                return i
    return None
...
try:
    print("\n==================================================")
    print(" 왼손 컨트롤러를 편한 위치와 방향으로 들고 대기하세요. (영점 조절)")
    ...
    while True:
        ...
        left_idx = get_left_controller_idx(vr_system)
        if left_idx is None:
            time.sleep(0.1)
            continue
        ...
        if not poses[left_idx].bPoseIsValid:
        ...
        trigger = get_trigger_value(vr_system, left_idx)
